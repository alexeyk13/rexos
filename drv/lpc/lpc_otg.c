...
static inline EP* ep_data(USB_PORT_TYPE port, SHARED_OTG_DRV* drv, unsigned int num)
{
    return (num & USB_EP_IN) ? (drv->otg.otg[port]->in[USB_EP_NUM(num)]) : (drv->otg.otg[port]->out[USB_EP_NUM(num)]);
}
...
static inline void lpc_otg_reset(USB_PORT_TYPE port, SHARED_OTG_DRV* drv)
{
    IPC ipc;
    ipc.process = drv->otg.otg[port]->device;
    ipc.cmd = HAL_CMD(HAL_USB, USB_RESET);
    ipc.param1 = USB_HANDLE(port, USB_HANDLE_DEVICE);
    ipc.param2 = drv->otg.otg[port]->speed;
    ipc_ipost(&ipc);
}

static inline void lpc_otg_suspend(USB_PORT_TYPE port, SHARED_OTG_DRV* drv)
{
    IPC ipc;
    ipc.process = drv->otg.otg[port]->device;
...
static inline void lpc_otg_wakeup(USB_PORT_TYPE port, SHARED_OTG_DRV* drv)
{
    IPC ipc;
    ipc.process = drv->otg.otg[port]->device;
...
static inline void lpc_otg_setup(USB_PORT_TYPE port, SHARED_OTG_DRV* drv)
{
    IPC ipc;
    ipc.process = drv->otg.otg[port]->device;
...
static void lpc_otg_on_isr(int vector, void* param)
{
    ...
    {
        lpc_otg_bus_reset(port, drv);

        drv->otg.otg[port]->suspended = false;
        __USB_REGS[port]->USBSTS_D = USB0_USBSTS_D_URI_Msk;
    }
    if (__USB_REGS[port]->USBSTS_D & USB0_USBSTS_D_SLI_Msk)
    {
        lpc_otg_suspend(port, drv);
        drv->otg.otg[port]->suspended = true;
        __USB_REGS[port]->USBSTS_D = USB0_USBSTS_D_SLI_Msk;
    }
    if (__USB_REGS[port]->USBSTS_D & USB0_USBSTS_D_PCI_Msk)
    {
        drv->otg.otg[port]->speed = __USB_REGS[port]->PORTSC1_D & USB0_PORTSC1_D_HSP_Msk ? USB_HIGH_SPEED : USB_FULL_SPEED;
        __USB_REGS[port]->USBSTS_D = USB0_USBSTS_D_PCI_Msk;
        if (drv->otg.otg[port]->suspended)
        {
            drv->otg.otg[port]->suspended = false;
            lpc_otg_wakeup(port, drv);
        }
        else
            lpc_otg_reset(port, drv);
    }
...
static bool lpc_otg_ep_flush(USB_PORT_TYPE port, SHARED_OTG_DRV* drv, int num)
{
    ...
    if (ep->io != NULL)
    {
        io_complete_ex(drv->otg.otg[port]->device, HAL_IO_CMD(HAL_USB, (num & USB_EP_IN) ? IPC_WRITE : IPC_READ), USB_HANDLE(port, num), ep->io, ERROR_IO_CANCELLED);
        ep->io = NULL;
    }
    return true;
}

static inline void lpc_otg_ep_set_stall(USB_PORT_TYPE port, SHARED_OTG_DRV* drv, int num)
{
    if (!lpc_otg_ep_flush(port, drv, num))
        return;
    EP_CTRL(port)[USB_EP_NUM(num)] |= USB0_ENDPTCTRL_S_Msk << ((num & USB_EP_IN) ? 16 : 0);
}

static inline void lpc_otg_ep_clear_stall(USB_PORT_TYPE port, SHARED_OTG_DRV* drv, int num)
{
    if (!lpc_otg_ep_flush(port, drv, num))
        return;
    EP_CTRL(port)[USB_EP_NUM(num)] &= ~(USB0_ENDPTCTRL_S_Msk << ((num & USB_EP_IN) ? 16 : 0));
...
static inline void lpc_otg_open_ep(USB_PORT_TYPE port, SHARED_OTG_DRV* drv, int num, USB_EP_TYPE type, unsigned int size)
{
    unsigned int reg;
    DQH* dqh;
    if (ep_data(port, drv, num) != NULL)
    {
...
static void lpc_otg_close_ep(USB_PORT_TYPE port, SHARED_OTG_DRV* drv, int num)
{
    if (!lpc_otg_ep_flush(port, drv, num))
        return;
    ...
    EP* ep = ep_data(port, drv, num);
    free(ep);
    num & USB_EP_IN ? (drv->otg.otg[port]->in[USB_EP_NUM(num)] = NULL) : (drv->otg.otg[port]->out[USB_EP_NUM(num)] = NULL);
}
...
static inline void lpc_otg_open_device(USB_PORT_TYPE port, SHARED_OTG_DRV* drv, HANDLE device)
{
    ...
    drv->otg.otg[port] = malloc(sizeof(OTG_TYPE));
    if (drv->otg.otg[port] == NULL)
        return;
    int i;
    for (i = 0; i < USB_EP_COUNT_MAX; ++i)
    {
        drv->otg.otg[port]->out[i] = NULL;
        drv->otg.otg[port]->in[i] = NULL;
    }
    drv->otg.otg[port]->device = device;
    drv->otg.otg[port]->suspended = false;
    drv->otg.otg[port]->speed = USB_FULL_SPEED;
    ...
    //enable interrupts
    irq_register(__USB_VECTORS[port], lpc_otg_on_isr, drv);
...
static inline void lpc_otg_close_device(USB_PORT_TYPE port, SHARED_OTG_DRV* drv)
{
    int i;
    //disable interrupts
    ...
    for (i = 0; i < USB_EP_COUNT_MAX; ++i)
    {
        if (drv->otg.otg[port]->out[i] != NULL)
            lpc_otg_close_ep(port, drv, i);
        if (drv->otg.otg[port]->in[i] != NULL)
            lpc_otg_close_ep(port, drv, USB_EP_IN | i);
    }
    //Mask all interrupts
    ...
    //free object
    free(drv->otg.otg[port]);
    drv->otg.otg[port] = NULL;
}

static inline void lpc_otg_set_address(USB_PORT_TYPE port, SHARED_OTG_DRV* drv, int addr)
{
...
static inline void lpc_otg_set_test_mode(USB_PORT_TYPE port, SHARED_OTG_DRV* drv, USB_TEST_MODES test_mode)
{
