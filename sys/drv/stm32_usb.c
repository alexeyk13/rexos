...
void stm32_usb_ep_enable(USB* usb, int num, int mps, USB_EP_TYPE type)
{
    ...
    if (num & USB_EP_IN)
    {
        ...
        if (USB_EP_NUM(num))
            OTG_FS_GENERAL->DIEPTXF[USB_EP_NUM(num) - 1] = ((mps / 4)  << OTG_FS_GENERAL_TX0FSIZ_TX0FD_POS) | (fifo_used | OTG_FS_GENERAL_TX0FSIZ_TX0FSA_POS);
        else
...
static inline void stm32_usb_rx_fifo(USB* usb)
{
    ...
    if (pktsts == OTG_FS_GENERAL_RXSTSR_PKTSTS_SETUP_RX || pktsts == OTG_FS_GENERAL_RXSTSR_PKTSTS_OUT_RX)
    {
        ...
        else if (ep->size)
        {
            ...
            {
                ep->io_active = false;
                stm32_usb_return_block(usb, num, USB_EP_READ_COMPLETE, ep->processed);
            }
            else
...
static inline void stm32_usb_write_chunk(USB* usb, int num)
{
    ...
    if (size)
    {
        ...
        {
            ep->io_active = false;
            stm32_usb_return_block(usb, num, USB_EP_WRITE_COMPLETE, ep->processed);
        }
        else
            OTG_FS_DEVICE->EIPEMPMSK |= 1 << USB_EP_NUM(num);
    }
