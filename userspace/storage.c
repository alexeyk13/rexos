...
static void storage_write_internal(HAL hal, HANDLE process, IO* io, unsigned int sector, unsigned int count, unsigned int flags)
{
    STORAGE_STACK* stack = io_push(io, sizeof(STORAGE_STACK));
    stack->sector = sector;
    stack->count = count;
...
static bool storage_write_sync_internal(HAL hal, HANDLE process, IO* io, unsigned int sector, unsigned int count, unsigned int flags)
{
    STORAGE_STACK* stack = io_push(io, sizeof(STORAGE_STACK));
    stack->sector = sector;
    stack->count = count;
...
{
    storage_write_internal(hal, process, io, sector, io->data_size / STORAGE_SECTOR_SIZE, STORAGE_FLAG_WRITE);
}

bool storage_write_sync(HAL hal, HANDLE process, IO* io, unsigned int sector)
{
    return storage_write_sync_internal(hal, process, io, sector, io->data_size / STORAGE_SECTOR_SIZE, STORAGE_FLAG_WRITE);
}

void storage_erase(HAL hal, HANDLE process, IO* io, unsigned int sector, unsigned int count)
{
    storage_write_internal(hal, process, io, sector, count, 0);
}

bool storage_erase_sync(HAL hal, HANDLE process, IO* io, unsigned int sector, unsigned int count)
{
    return storage_write_sync_internal(hal, process, io, sector, count, 0);
}

void storage_verify(HAL hal, HANDLE process, IO* io, unsigned int sector)
{
    storage_write_internal(hal, process, io, sector, io->data_size / STORAGE_SECTOR_SIZE, STORAGE_FLAG_VERIFY);
}

bool storage_verify_sync(HAL hal, HANDLE process, IO* io, unsigned int sector)
{
    return storage_write_sync_internal(hal, process, io, sector, io->data_size / STORAGE_SECTOR_SIZE, STORAGE_FLAG_VERIFY);
}

void storage_write_verify(HAL hal, HANDLE process, IO* io, unsigned int sector)
{
    storage_write_internal(hal, process, io, sector, io->data_size / STORAGE_SECTOR_SIZE, STORAGE_FLAG_WRITE | STORAGE_FLAG_VERIFY);
}

bool storage_write_verify_sync(HAL hal, HANDLE process, IO* io, unsigned int sector)
{
    return storage_write_sync_internal(hal, process, io, sector, io->data_size / STORAGE_SECTOR_SIZE, STORAGE_FLAG_WRITE | STORAGE_FLAG_VERIFY);
}
